...
// The new products we mistakenly added at the end of rawBlogPosts
const newProducts = [
  {
...
const customProductsString = newProducts.map(p => ",\n{\n" + JSON.stringify(p, null, 2).slice(2, -2) + "\n}").join('').replace(/"image": "data/g, '"image": `data').replace(/svg\+xml;base64,.*"/g, match => match.slice(0, -1) + "`");

// Find exactly where the mistake was
const mistakeString = newProducts.map(p => ",\n" + JSON.stringify(p, null, 2).replace(/"image": "data/g, '"image": `data').replace(/svg\+xml;base64,.*"/g, match => match.slice(0, -1) + "`")).join('');

content = content.replace(mistakeString, ''); // remove from blogPosts

// Add it to rawProducts instead
const properInsertion = content.indexOf('];\n\nexport const products');
content = content.slice(0, properInsertion) + customProductsString + content.slice(properInsertion);
